...
namespace engine {
    ...
    inline vec<TypeXY, TypeZ>::vec() noexcept : x(0), y(0), z(0) {}

    template <typename TypeXY, typename TypeZ>
    engine::vec<TypeXY, TypeZ>::vec(TypeXY x, TypeXY y, TypeZ z) : x(x), y(y), z(z) {}

    template <typename TypeXY, typename TypeZ>
    engine::vec<TypeXY, TypeZ>::vec(TypeXY x, TypeXY y) : x(x), y(y), z(char(0)) {}
    ...
    template <typename TypeXY, typename TypeZ>
    vec<TypeXY, TypeZ>& engine::vec<TypeXY, TypeZ>::operator= (vec& newData) noexcept {
    ...
    template <typename TypeXY, typename TypeZ>
    inline double vec<TypeXY, TypeZ>::getDist() const noexcept {return sqrt(this -> getLen());}
    template <typename TypeXY, typename TypeZ>
    inline double vec<TypeXY, TypeZ>::getDist(const vec& centerData) const noexcept {return sqrt(this -> getLen(centerData));}
    ...
    template <typename TypeXY, typename TypeZ>
    template <typename any>
    inline engine::vec<TypeXY, TypeZ> engine::vec<TypeXY, TypeZ>::operator* (const any& integerNum) const noexcept {return vec(x*integerNum,   y*integerNum,   z*integerNum);  }
    template <typename TypeXY, typename TypeZ>
    template <typename any>
    inline engine::vec<TypeXY, TypeZ> engine::vec<TypeXY, TypeZ>::operator/ (const any& integerNum) const          {return vec(x/integerNum,   y/integerNum,   z/integerNum);  }
    template <typename TypeXY, typename TypeZ>
    inline engine::vec<TypeXY, TypeZ> engine::vec<TypeXY, TypeZ>::operator* (const vec& vectorData) const noexcept {return vec(x*vectorData.x, y*vectorData.y, z*vectorData.z);}
    template <typename TypeXY, typename TypeZ>
    inline engine::vec<TypeXY, TypeZ> engine::vec<TypeXY, TypeZ>::operator/ (const vec& vectorData) const          {return vec(x/vectorData.x, y/vectorData.y, z/vectorData.z);}              //!!!!! Деление 0 на 0! Необходимо выдавать ошибку bad value(vector.[INCORRECT_VALUE_CORD])
    template <typename TypeXY, typename TypeZ>
    inline engine::vec<TypeXY, TypeZ> engine::vec<TypeXY, TypeZ>::operator+ (const vec& vectorData) const noexcept {return vec(x+vectorData.x, y-vectorData.x, z-vectorData.z);}
    template <typename TypeXY, typename TypeZ>
    inline engine::vec<TypeXY, TypeZ> engine::vec<TypeXY, TypeZ>::operator- (const vec& vectorData) const noexcept {return vec(x-vectorData.x, y-vectorData.y, z-vectorData.z);}
    ...
    template <typename TypeXY, typename TypeZ>
    inline bool engine::vec<TypeXY, TypeZ>::operator== (const vec& vectorData) const noexcept {return x == vectorData.x && y == vectorData.y && z == vectorData.z;}
    template <typename TypeXY, typename TypeZ>
    inline bool engine::vec<TypeXY, TypeZ>::operator!= (const vec& vectorData) const noexcept {return !(*this == vectorData);}
};
template <typename TypeXY, typename TypeZ>
